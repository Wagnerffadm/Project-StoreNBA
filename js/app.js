...
var CELULAR_EMPRESA = `5547999337456`;
...
loja.eventos = {
    ...
    init: () => {
        ...
        loja.metodos.carregarBotaoReserva();
    }
}
...
loja.metodos = {
    ...
    obterItensLoja: (categoria = 'lakers', viewmore = false) => {
        ...
        $.each(filtro, (i, e) => {
            ...
            let temp = loja.templates.item.replace(/\${img}/g, e.img) /*  /\${img}/g - rejex */
    ...
    verMais: () => {
        ...
        var ativo = $ (".container-menu a.active").attr('id').split('menu-')[1]; //attr= menu-lakers - split= quebra o MENU
        loja.metodos.obterItensLoja(ativo, true);

        $("#btnVerMais").addClass('hidden');

    },
    // diminuir a quantidade do item da loja
    ...
    // aumentar a quantidade do item da loja
    ...
    // adicionar ao carrinho o item da loja
    adicionarAoCarrinho: (id) => {
        ...
        if (qntdAtual > 0) {
            ...
            if (item.length > 0) {
                ...
                //validar se já existe este istem no carrinho
    ...
    voltarEtapa: () => {
        ...
        loja.metodos.carregarEtapa(etapa - 1);
    ...
    carregarCarrinho: () => {
        ...
        if (MEU_CARRINHO.length > 0) {
            ...
            $.each(MEU_CARRINHO, (i, e) => {
                ...
                let temp = loja.templates.itemCarrinho.replace(/\${img}/g, e.img) /* /\${img}/g - rejex */
                .replace(/\${nome}/g, e.name)
                .replace(/\${preco}/g, e.price.toFixed(2).replace('.', ','))
                .replace(/\${id}/g, e.id)
    ...
    diminuirQuantidadeCarrinho: (id) => {
        ...
        if (qntdAtual > 1) {
            ...
            loja.metodos.removerItemCarrinho(id)
        }
    ...
    //BOTAO DE "XIS" para remover quantidade do item no carrinho
    ...
    atualizarCarrinho: (id, qntd) => {
        ...
        //atualiza od valores em R$ tatais do carrinho
        loja.metodos.carregarValores();
            ...
            if(validacep.test(cep)) {
                ...
                $.getJSON("https://viacep.com.br/ws/" + cep + "/json/?callback=?", function(dados){
                    ...
                    if(!("erro" in dados)) {
                        ...
                        loja.metodos.mensagem('CEP não encontrado. Preencha as informações manualmente.');
                    $("#txtEndereco").focus();
    ...
    carregarResumo: () => {
        ...
        $.each(MEU_CARRINHO, (i, e) => {
            ...
            let temp = loja.templates.itemResumo.replace(/\${img}/g, e.img) /* /\${img}/g - rejex */
                .replace(/\${nome}/g, e.name)
                .replace(/\${preco}/g, e.price.toFixed(2).replace('.', ','))
                .replace(/\${qntd}/g, e.qntd)
    ...
    abrirDepoimento: (depoimento) => {
        ...
        $("#bntDepoimento-1").removeClass('active');
        $("#bntDepoimento-2").removeClass('active');
        $("#bntDepoimento-3").removeClass('active');

        $("#depoimento-" + depoimento).removeClass('hidden');
        $("#bntDepoimento-" + depoimento).addClass('active');
...
loja.templates = {
        ...
        <div class="col-12 col-lg-3 col-md-3 col-sm-6 mb-5 animated fadeInUp">
            <div class="card card-item" id="\${id}">
                    ...
                    <div class="add-carrinho">
                        ...
                        <span class="btn btn-add" onclick="loja.metodos.adicionarAoCarrinho('\${id}')"><i class="fa fa-shopping-bag"></i></span>
                    </div>
            </div>                           
        </div>
    `,
    ...
    itemCarrinho: `
            ...
            <div class="add-carrinho">
                ...
                <span class="btn btn-remove no-mobile" onclick="loja.metodos.removerItemCarrinho('\${id}')"><i class="fas fa-times"></i></span>
            </div>
        </div> 
    `,
